console.log('Loading story-presenter.js');
    ...
    async loadStories() {
        try {
            ...
            if (this.authModel.isAuthenticated()) {
                if (navigator.onLine) {
                    // Online: fetch from API with timeout
                    try {
                        ...
                        const response = await Promise.race([
                            this.apiService.getStories(this.authModel.getToken(), 1, 50, 1),
                            timeoutPromise
                        ]);
                        ...
                        // If timeout, show specific message
                        if (error.message === 'Request timeout') {
                            this.view.showError('Loading is taking longer than expected. Showing cached content.');
                        }
                    }
                } else {
            ...
            // Try cached stories as last resort
            if (this.offlineManager) {
                try {
                    const cachedStories = await this.offlineManager.getCachedStories();
                    if (cachedStories.length > 0) {
                        this.model.setStories(cachedStories);
                        this.view.showSuccess(`Loaded ${cachedStories.length} cached stories (recovery mode)`);
                        return;
                    }
                } catch (cacheError) {
            ...
            this.view.showError('Failed to load stories. Please check your connection and try again.');
        }
    }
