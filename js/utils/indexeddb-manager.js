...
export class IndexedDBManager {
    ...
    try {
        ...
        const storiesToSave = stories.map(story => ({
            ...story,
            id: story.id || Date.now().toString(), // Pastikan selalu ada ID
            hasLocation: !!(story.lat && story.lon),
            cachedAt: new Date().toISOString()
        }));

        // Gunakan transaction yang sama untuk clear dan add
        return new Promise((resolve, reject) => {
            if (!this.db) {
                reject(new Error('Database not initialized'));
                return;
            ...
            clearRequest.onsuccess = () => {
                // Add new stories
                const addRequests = storiesToSave.map(story => {
                    return new Promise((resolveAdd, rejectAdd) => {
                        const request = store.add(story);
                        request.onsuccess = () => resolveAdd();
                        request.onerror = () => rejectAdd(new Error(`Failed to add story ${story.id}`));
                    });
                });
            ...
            clearRequest.onerror = () => {
                reject(new Error('Failed to clear stories store'));
            };
    ...
    async addToFavorites(story) {
        try {
            ...
            const storyId = String(story.id || story.storyId || Date.now());
            
            const favorite = {
                storyId: storyId,
                story: {
                    ...story,
                    id: storyId // Ensure story has consistent id
                },
                addedAt: new Date().toISOString()
            };

            await this.update(this.stores.favorites, favorite);
    ...
    async removeFromFavorites(storyId) {
        try {
            ...
            await this.delete(this.stores.favorites, id);
    ...
    async isFavorite(storyId) {
        try {
            // Convert to string to ensure consistency
            const id = String(storyId);
            const favorite = await this.get(this.stores.favorites, id);
            return !!favorite;
        } catch (error) {
            return false;
        }
