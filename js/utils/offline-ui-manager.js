// Offline UI Manager - Mengelola UI untuk fitur offline (favorites, drafts, sync)
export class OfflineUIManager {
    ...
    addOfflineNavigation() {
        ...
        offlineMenuItem.innerHTML = `
            <a href="#offline" class="nav-link" id="offline-link">
                <i class="fas fa-heart"></i> Offline
            </a>
        `;
    ...
    async handleRemoveFavorite(storyId) {
        ...
        try {
            const result = await this.offlineManager.removeFromFavorites(storyId);
