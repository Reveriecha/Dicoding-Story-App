...
export class OfflineManager {
    ...
    async checkAndSyncPendingStories() {
        try {
            ...
            if (window.app && window.app.authPresenter) {
                ...
                if (token) {
                    ...
                    if (pendingCount > 0) {
                        console.log(`Found ${pendingCount} pending operations on startup...`);
    ...
    async handleOnline() {
        ...
        // Auto-sync pending stories when coming back online
        setTimeout(async () => {
            try {
                // Check if user is authenticated
                if (window.app && window.app.authPresenter) {
                    ...
                    if (token) {
                        const pendingCount = await this.getPendingOperationsCount();
                        if (pendingCount > 0) {
                            console.log(`Found ${pendingCount} pending operations, starting auto-sync...`);
                            
                            // Show notification to user
                            this.notifyAutoSyncStart(pendingCount);
                            
                            // Perform sync
                            const result = await this.syncPendingStories(token);
                            
                            if (result.success && result.syncedCount > 0) {
                                this.notifyAutoSyncComplete(result.syncedCount);
                                
                                // Refresh stories
                                if (window.app && window.app.storyPresenter) {
                                    await window.app.storyPresenter.loadStories();
                                }
                            } else if (result.failedCount > 0) {
                                this.notifyAutoSyncPartial(result.syncedCount, result.failedCount);
                            }
                        }
                    }
                }
        ...
        }, 2000); // Wait 2 seconds before attempting sync
